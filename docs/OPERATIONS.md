...
- `REVIEW_AGENT_ACTIVE_JOB_LIMIT` — default `100`. Maximum queued, leased, and
  publication-waiting reviews. At capacity, the App worker retains the accepted
  webhook intent and retries admission without consuming processing attempts.
- `REVIEW_AGENT_GITHUB_APP_ADMISSION_MAX_AGE_SECONDS` — default `86400` (24 hours).
  Review requests that reach the App worker after this deadline fail with
  `review_admission_expired`. This bounds active intent retention during capacity
  waits or an App worker outage. Changing the setting applies to pending requests.
- `REVIEW_AGENT_GITHUB_APP_CAPACITY_RETRY_SECONDS` — default `300` (five minutes).
  Interval between capacity checks. Each attempt reauthorizes the request through
  GitHub; the default permits about 24 permission/snapshot reads per hour per
  waiting review, plus token exchanges when needed.
...
Capacity waits appear in App worker logs as `status=received` with
`reason=review_waiting_for_capacity`. Admission rechecks current authorization and
...
GitHub cooldowns retain the `Retry-After` deadline (seconds or HTTP date) and,
when the remaining quota is zero, the `X-RateLimit-Reset` time. App admission,
publication, and failure-status retries wait until both the provider deadline
and the normal retry delay have passed. PostgreSQL stores that time, so a process
restart or explicit publication retry cannot bypass it. Waiting consumes no
additional attempts; actual failed calls still use the bounded attempt budget.
Rate limits without usable timing wait at least one minute. An ordinary transient
error with unusable timing keeps its existing transport retry behavior. Review
admission wakes at its configured age limit to expire without another GitHub call,
even if the provider deadline is later. Valid provider deadlines are not shortened.
To abandon a delayed publication, use `review-agent-admin jobs cancel <id>`; the
publisher retires its terminal run without waiting for the cooldown. A newer
successful review suppresses an older pending failure status, including one still
in cooldown. Feedback acknowledgements honor the provider deadline and currently
have no separate age limit. Permanent denials keep their terminal behavior. A failed
failure-status delivery leaves the publisher running after its retry or terminal
result is saved.
...
Migration 029 separates code and documentation review identities. Before applying
it, pause admission and drain active reviews and publication deliveries, then
stop all application services, including `review-admin`. Deploy every service
from the same source revision before resuming work. Older workers must not run
against this schema alongside newer workers: their queries assume one review
purpose per pull request. Existing records remain code reviews with their IDs,
finding references, and decisions preserved. If rollback is required after
documentation records have been created, restore the pre-upgrade database backup
with its matching application revision; do not remove purpose columns in place.
...
Hermes v2026.8.31 owns the supported provider integrations. The Review Agent
