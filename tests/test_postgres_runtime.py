...
class PostgreSQLRuntimeTests(unittest.TestCase):
    ...
    def test_explicit_open_proves_session_migrations_and_metrics(self) -> None:
        ...
        self.assertEqual(readiness.applied_migration_version, 20)
    ...
    def test_previous_image_accepts_a_database_with_a_newer_migration(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            for name in ("019_admin_run_actions.sql", "020_deployment_settings.sql"):
                shutil.copy2(runner.MIGRATION_DIRECTORY / name, current / name)
            (current / "021_newer.sql").write_text(
        ...
        self.assertEqual(readiness.applied_migration_version, 21)
