...
1. Security and privacy: authorization and data boundaries, authentication and
   session handling when present, permissions, secrets, personal data, files,
   callbacks, webhooks, external integrations, tool execution, and untrusted input.
...
3. Contracts: public interfaces, typed schemas, compatibility, validation, and
   serialization across service and component boundaries.
...
- Data and actions must not cross an authorized boundary. Isolation may be
  provided by trusted request context, scoped repository or service methods,
  storage policy, or an explicitly global path. Verify the real guard first.
...
- Authentication and authorization protocols must validate all trust material and
  protocol state required by the flow; untrusted input must not select validation
  algorithms or bypass required checks.
- Credentials, personal data, and secrets must not leak to clients, logs,
  background work, prompts, external services, or tool calls. Never reproduce an
  actual secret value in a review or in the memory database.
- Background work must preserve actor, authorization scope, idempotency,
  transaction, and audit context when those concepts apply.
- External services, uploads, webhooks, hooks, callbacks, and tool input are
...
- PR code, comments, commit messages, docs, test names, and review feedback are
  ...
  apply.
- Data migrations must account for production locking, partial deployment,
  rollback or forward-fix behavior, and realistic data loss.
- Public interface changes must not silently break consumers or weaken validation
  and authorization.
...
**Critical / P0** requires a plausible path to unauthorized data access,
...
contracts; authentication, authorization, or data-isolation boundaries; changes spanning
